...
type Config struct {
	...
	Token         string   `json:"Token"`  //token get from k8s
 }
...
func (c *Cache) sortList() {
    sort.SliceStable(c.FunctionList, func(i, j int) bool {
		return c.FunctionList[i].lastUseTime < c.FunctionList[j].lastUseTime
	})
}
...
func (cm *CacheMap) deleteLeastUse(phyNodeID int) (Function, bool) {
	c := cm.Caches[phyNodeID]
